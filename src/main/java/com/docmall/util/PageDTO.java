...
public class PageDTO {
	...
	/*
	 한 페이지에 5개씩 출력
	 전체 데이터 개수 : 일정패턴. 25, 50, 75, 100
	 전체데이터개수 61
	 
	 총 페이지수 = 전체데이터 개수(61) / 페이지 출력건수 (5)
	...
	//	super(); 생략가능
		...
		// 로직을 만들어서, 위의 4개필드에 해당하는 정보를 저장한다.
		
		this.total = total;
		this.cri = cri;
		
		// 1block당 페이지번호를 5개씩 출력할 경우
		// endPage : 5.0, 5
		// startPage : endPage - 4
		// 1block당 페이지번호를 5개씩 출력할 경우
		// endPage : 10.0, 10
		// startPage : endPage - 9
		this.endPage = (int) (Math.ceil(cri.getPageNum() / 5.0)) * 5; // 1 block에서 페이지번호를 선택하게 되면.
		...
		// 총데이터 개수에 따라서 위의 공식이 적용이 되지 않는다.(예외)
		// 아래구문에서 조건식 true인 경우에는 3block에서 endPage의 값이 정확한 값이 아니기때문에, 실제 데이터의 총 개수를 구하여, endPage를 구하자.
		// 전체데이터 개수에 따라서 실제 마지막 페이지를 구한다.
